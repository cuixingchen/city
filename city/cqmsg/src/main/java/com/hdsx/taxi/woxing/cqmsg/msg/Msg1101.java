...
//		
//		int dl = Converter.getBytes(order.getPassengerInfo().getPassengerName()).length+1
//				 +1
...
//	}
	...
	protected byte[] bodytoBytes() {
		...
		// begin 司机信息  
		byte[] opin = Converter.getBytes(order.getPassengerInfo().getPassengerName());
		b.put(opin);
		b.put((byte) 0x00);
		
		b.put(order.getPassengerInfo().getPassengerSex());
		
		byte[] opip = Converter.getBytes(order.getPassengerInfo().getPassengerPhone());
		b.put(opip);
		b.put((byte) 0x00);
		// end 司机信息
	...
	protected boolean bodyfrombytes(byte[] b) {
		...
		pi.setPassengerName(Converter.toGBKString(b, offset,stringEndIdx-offset));
		offset = stringEndIdx;
		offset+=1;
		pi.setPassengerSex(bf.get(offset));
		offset += 1 ;
		stringEndIdx=FindEndFlag.getFirstStringEndFlag(b, offset);
		pi.setPassengerPhone(Converter.toGBKString(b, offset,stringEndIdx-offset));
		offset = stringEndIdx;
		offset+=1;
		...
		offset += 7 ;
		stringEndIdx=FindEndFlag.getFirstStringEndFlag(b, offset);
		order.setAddress(Converter.toGBKString(b, offset,stringEndIdx-offset));
		offset = stringEndIdx ;
		offset += 1 ;
		...
		offset += 4 ;
		stringEndIdx=FindEndFlag.getFirstStringEndFlag(b, offset);
		order.setDestination(Converter.toGBKString(b, offset,stringEndIdx-offset));
		offset = stringEndIdx;
		offset += 1 ;
